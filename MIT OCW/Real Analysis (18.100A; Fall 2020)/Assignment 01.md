...
To prove that $f$ is injective, suppose $x \ne y$. By our Theorem, we know $x$ and $y$ may be expressed uniquely (and distinctly) as a product of primes over another product of primes (for brevity, we assume the empty product is $1$). That is to say, if $x = $p_1^{r_1} \cdots p_N^{r_N} / q_1^{s_1} \cdots q_M^{s_M}$ and $y = p'_1^{r'_1} \cdots p'_K^{r'_K}$
