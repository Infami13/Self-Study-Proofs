...
*Proof*. To prove $f$ is bijective, we start by showing $f$ is surjective. Suppose $n \in \mathbb{N}$, then $n = 1$ or $n$ is a product of primes by our Theorem. If $n = 1$, then $f(1) = 1$ by our definition of $f$. Otherwise, by our Theorem again, $n$ may be expressed uniquely as a product of primes $\pi_1^{\rho_1} \cdots \pi_K^{\rho_K}$ for $\pi_1 < \pi_2 < ... < \pi_K$. Given multiplication is commutative for the natural numbers, we may arrange the prime power factors of $n$ such that the first $N$ prime power factors have even exponents (where $\rho_i = 2r_i$ for some $r_i$) and the last $K - N = M$ prime power factors have odd exponents (where $\rho_i = 2s_i -1$ for some $s_i$). That is, $n = p_1^{2r_1} \cdots p_N^{2r_N} q_1^{2s_1 - 1} \cdots q_M^{2s_M - 1}$. We then choose $q = \frac{p_1^{r_1} \cdots p_N^{r_N}}{q_1^{s_1} \cdots q_M^{s_M}}$ so that $f(q) = p_1^{2r_1} \cdots p_N^{2r_N} q_1^{2s_1 - 1} \cdots q_M^{2s_M - 1} = n$ as desired.
