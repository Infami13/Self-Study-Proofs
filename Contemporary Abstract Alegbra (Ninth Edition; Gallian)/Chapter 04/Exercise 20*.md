...
Take $g \in G$. We know that $g^{35} = e$. By Theorem Corollary 2 of 4.1, we know if $g^{35}$, then $|g|$ divides $35$. For this to be possbile, the order of $g$ must be 1, 5, 7, or 35. That is to say each element of $G$ has order 1, 5, 7, or 35.
...
Suppose $G$ has a mix of non-identity elements with order 5 and 7. Then we may select such elements $x$ and $y$ such that $|x|=5$ and $|y|=7$. Again, as $xy \in G$ it must be the case $xy$ has order 1, 5, 7, or 35. We know $xy \ne e$ as $x$ and $y$ have different orders, so $|xy| \ne 1$.† However, given $G$ is commutative, we also see that $(xy)^5 = x^5y^5 = ey^5 = y^5$ and $(xy)^7 = x^7y^7 = x^5x^2e = ex^2e = x^2$. We are left with $|xy|=35$. 
...
The arguments does not work if $35$ is replaced with $33 = 3 \cdot 11$ as there may be the case where a group has 1 identity element and all other $16\phi(3) = 16\cdot2 = 32$ elements of order 3.

† - If $|x| = n$, then $x^n = xx^{n-1} = e$......
